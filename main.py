            ...
            if tracker_init:
                if patient_bbox is not None:
                    with open(output_dir, "a") as f:
                       print(f"{t}|{ui_state}|{int(tracker_init)}|{d}|{a}|{fps}|{len(confs)}|{patient_bbox[0]}|{patient_bbox[1]}|{patient_bbox[2]}|{patient_bbox[3]}", file=f)
            else:
                with open(output_dir, "a") as f:
                    print(f"{t}|{ui_state}|{int(tracker_init)}|{d}|{a}|{fps}|{len(confs)}|None|None|None|None", file=f)
